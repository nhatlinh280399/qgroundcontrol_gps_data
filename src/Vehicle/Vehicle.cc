...
void Vehicle::_handleGpsRawInt(mavlink_message_t& message)
{
    ...
    if (gpsRawInt.fix_type >= GPS_FIX_TYPE_3D_FIX) {
        if (!_globalPositionIntMessageAvailable) {
            ...
            latx = abs(gpsRawInt.lat / (int)1E7) ;
            lonx = abs(gpsRawInt.lon / (int)1E7);
            laty = abs(gpsRawInt.lat % int(1E7));
            lony = abs(gpsRawInt.lon % int(1E7));
            altitudex = gpsRawInt.alt  / 1000.0;

           

            //write gps to a json file
            fp = fopen("/home/linh/LabJob/WebPage_Drone_apigoogle/data.json", "w");

            fprintf(fp, "'{ \"Lat\": %d.%07d, \"Lng\": %d.%07d, \"Alt\": %d }'",latx, laty, lonx, lony, altitudex );
            fclose(fp);
